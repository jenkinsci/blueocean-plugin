...
describe('DashboardCards', () => {
    ...
    it('sorts the cards by status, then by name', () => {
        ...
        assert.equal(cards.at(6).props().status, 'QUEUED');

        assert.equal(cards.at(7).props().status, 'SUCCESS');
        assert.equal(cards.at(7).props().organization, 'jankins');
        assert.equal(cards.at(7).props().pipeline, 'jdl1');
        assert.equal(cards.at(7).props().branch, 'docker-test');

        assert.equal(cards.at(8).props().status, 'SUCCESS');
        assert.equal(cards.at(8).props().organization, 'jenkins');
        assert.equal(cards.at(8).props().pipeline, 'jenkinsfile-experiments');
        assert.equal(cards.at(8).props().branch, 'master');

        assert.equal(cards.at(9).props().status, 'SUCCESS');
        assert.equal(cards.at(9).props().organization, 'jenkins');
        assert.equal(cards.at(9).props().pipeline, 'jenkinsfile-experiments');
        assert.equal(cards.at(9).props().branch, 'test-branch-1');
    });
});
