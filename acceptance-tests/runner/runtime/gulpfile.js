...
if (process.argv.length === 4 && process.argv[2] === '--test') {
    gulp.task('default', shell.task('nightwatch --retries 5 --suiteRetries 2 ' + process.argv[3].toString(), {cwd: '../../'}));
} else {
    gulp.task('default', shell.task('nightwatch --retries 5 --suiteRetries 2', {cwd: '../../'}));
}
