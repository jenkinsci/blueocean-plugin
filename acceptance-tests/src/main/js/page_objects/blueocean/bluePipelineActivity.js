...
module.exports.commands = [
    {
        ...
        assertStageGraphShows: function() {
            ...
            this.waitForElementVisible('.PWGx-pipeline-node-selected');
            this.waitForElementVisible('.download-log-button');
            this.waitForElementVisible('.PWGx-pipeline-selection-highlight');
            // in Chrome, 'Visible' seems to fail but 'Present' works ok
            this.waitForElementPresent('.PWGx-pipeline-connector');
            this.waitForElementPresent('.PWGx-pipeline-node-hittarget');
