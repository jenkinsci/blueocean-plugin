 ...
 * @author Vivek Pandey
 */
public class GitPipelineUpdateRequest extends BluePipelineCreateRequest {
    ...
    public BluePipeline create(Reachable parent) throws IOException {
        return create((BluePipeline)parent);
    }
    
    @CheckForNull
    @SuppressWarnings("unchecked")
    public BluePipeline create(BluePipeline pipeline) throws IOException {
    ...
    private BranchSource getGitScmSource(MultiBranchProject mbp){
        ...
        if(scmConfig != null){
            ...
            if (!errors.isEmpty()){
                throw new ServiceException.BadRequestException(new ErrorMessage(400, "Failed to create Git pipeline")
                        .addAll(errors));
