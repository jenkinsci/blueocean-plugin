...
All changes should come via pull requests, never to controller directly. A pull request can be as a branch on this repo or from a forked one, doesn't matter. If a branch on this repo, name it topic/x feature/x or bug/x - depending on what you are doing. Once it is merged, please do delete the branch. 
...
All commits should be squashed before being merged to `controller` to preserve a sane history. The Merge button on the PR screen has been configured to do this automatically.
