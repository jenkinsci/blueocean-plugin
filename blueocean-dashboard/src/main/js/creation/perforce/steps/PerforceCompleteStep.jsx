...
import STATE from '../PerforceCreationState';
...
export default class PerforceCompleteStep extends React.Component {
    ...
    _getTitle(state, repo) {
        if (state === STATE.PENDING_CREATION_SAVING || state === STATE.PENDING_CREATION_EVENTS) {
            return t('creation.p4.pending.title');
        } else if (state === STATE.STEP_COMPLETE_SAVING_ERROR) {
            return t('creation.core.error.saving.pipeline');
    ...
    render() {
        ...
        const title = this._getTitle(flowManager.stateId, flowManager.selectedProject.id);
