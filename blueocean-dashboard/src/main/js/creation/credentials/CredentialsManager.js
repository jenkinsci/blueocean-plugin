...
export class CredentialsManager {
    ...
    listAllCredentials() {
        return this._api.listAllCredentials().then(result => this._listAllCredentialsComplete(result));
    }
    ...
    _storeCredentials(credentialList) {
        ...
        const systemSSH = credentialList.filter(item => item.id === SYSTEM_SSH_ID).pop();
        ...
        const filtered = credentialList.filter(item => item.id !== SYSTEM_SSH_ID);
    ...
    _createSystemSSHCredentialConditionally() {
        if (!this.systemSSHCredential) {
            ...
            return this.saveSystemSSHCredential().then(() => this.credentials);
        }
    ...
    saveSystemSSHCredential() {
        return this._api.saveSystemSSHCredential(SYSTEM_SSH_ID, SYSTEM_SSH_DESCRIPTION).then(cred => this._saveSystemSSHCredentialSuccess(cred));
    }
