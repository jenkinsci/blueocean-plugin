...
function transformToJSX() {
    ...
    if (jsExtensionsYAMLFile) {
        ...
        jsxFileContent += "require('@jenkins-cd/js-modules').import('jenkins-cd:js-extensions').onFulfilled(function(store) {\n";
        for (var i2 = 0; i2 < extensions.length; i2++) {
            var extension = extensions[i2];

            jsxFileContent += "    store.register('" + extension.extensionPoint + "', " + extension.importAs + ");\n";
        }
        jsxFileContent += "});";
