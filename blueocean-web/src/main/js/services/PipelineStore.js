...
class PipelineStore {
    ...
    createSequentialStage(name:string) {
        ...
        const newStage = createStage(name);
    ...
    createParallelStage(name:string, parentStage:StageInfo) {
        const updatedChildren = [...parentStage.children]; // Start with a shallow copy, we'll add one or two to this
        ...
        if (parentStage.children.length == 0) {
            ...
            const zerothStage = createStage(parentStage.name);
    ...
    deleteStep(step:StepInfo) {
        ...
        const oldStepsForStage = selectedStage.steps || [];
        const newStepsForStage = oldStepsForStage;
        let newSelectedStep;
        ...
        if (parent) {
            ...
            const newSelectedStepIdx = Math.min(stepIdx, newStepsForStage.length - 1);
    ...
    moveStep(stage, sourceNodeId, targetNodeId, targetType) {
        ...
        if (targetType === DragPosition.FIRST_CHILD || targetType === DragPosition.LAST_CHILD) {
            ...
            const targetIndex = targetArray.indexOf(targetStep);
